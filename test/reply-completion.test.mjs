...
test('completion never renders an empty stream before its final message exists',async()=>{
 for(const state of ['final','aborted','error']){
  const frames=[];
  const context={f:{type:'chat',sessionKey:'one',state,text:state==='final'?'こんにちは、私はゴンタです。':''},selected:'one',runId:'run',stream:'こんにちは、私はゴンタ',activity:null,historyGeneration:4,messages:[{role:'user',text:'自己紹介して'}],notice(){},refreshSessions(){},loadHistory(){throw Error('Must not replace the completed reply with a history request');}};
...
test('new stream data invalidates older history requests',async()=>{
 const context={f:{type:'chat',sessionKey:'one',state:'delta',runId:'run',text:'続き'},selected:'one',historyGeneration:3,renderMessages(){},updateComposer(){}};
 await vm.runInNewContext(`(async()=>{${handler}})()`,context);
...
test('history keeps a newer stream and can still recover a missed completion',async()=>{
 let renders=0;
 const context={selected:'one',historyGeneration:0,runId:'run',stream:'こんにちは、私はゴンタ',messages:[],notice(){},renderMessages(){renders++;},updateComposer(){},rpc:async()=>({messages:[],inFlightRun:{runId:'run',text:'こんにちは'}})};
