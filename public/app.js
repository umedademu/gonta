...
function updateComposer(){const enabled=connected&&gatewayReady&&!!selected;$('message').disabled=!enabled;$('send').disabled=!enabled||!!runId;$('message').placeholder=enabled?'Gontaに話しかける…':'まずはPCに接続してください';$('composer-state').textContent=runId?'Gontaが考えています…':enabled?'Enterで送信 · Shift + Enterで改行':'接続後にメッセージを送信できます';$('stop').hidden=!runId;}
