...
const conversationKey='agent:main:discord:channel:1472809918645010535';
...
for(const id of ['welcome-connect','connection-pill'])$(id).onclick=showSettings;
document.querySelectorAll('[data-close]').forEach(el=>el.onclick=()=>$(el.dataset.close).close());
function setStatus(status){backgroundActivity=status.backgroundActivity?.kind==='diary'?{kind:'diary'}:null;gatewayReady=!!status.gateway;document.body.classList.toggle('connected',connected&&gatewayReady);$('connection-label').textContent=connected?(gatewayReady?'接続中':'PCを確認'):'未接続';$('line-dialog').dataset.enabled=String(!!status.line);$('line-add').hidden=!status.lineBotId;if(status.lineBotId)$('line-add').href='https://line.me/R/ti/p/'+encodeURIComponent(status.lineBotId);updateComposer();}
...
async function connect(c){
 ...
 current.onmessage=async event=>{
  ...
  if(f.type==='ready'){
   ...
   try{await refreshSessions();await selectSession(conversationKey);}catch(e){notice(e.message);}
  }
...
$('disconnect').onclick=()=>{void memo.setCredential(null);sendQueues.clear();queuePauses.clear();editingQueued=null;historyReady=false;historyActive=false;++connectionAttempt;intentional=true;clearTimeout(reconnectTimer);ws?.close();sessionStorage.removeItem('gonta.connection');localStorage.removeItem('gonta.relay');localStorage.removeItem('gonta.remembered');remember.checked=false;credentials=null;connected=false;selected=null;sessionList=[];messages=[];runId=null;stream='';$('passcode').value='';$('relay-url').value='';setStatus({});renderMessages();$('settings').close();$('welcome').hidden=false;$('conversation-title').textContent='会話のつづき';notice('');};
async function refreshSessions(){sessionList=await rpc('sessions');}
async function selectSession(key){if(key!==conversationKey)throw Error('この画面では「何でも」の会話を開きます。');cancelQueueEdit();historyReady=false;historyActive=false;activity=null;selected=key;runId=null;stream='';messages=[];renderMessages();$('welcome').hidden=true;$('conversation-title').textContent='#何でも';updateComposer();await loadHistory();}
...
for(const id of ['welcome-line'])$(id).onclick=showLine;
