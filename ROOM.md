...
2026-09-20に通常姿 `public/room-gonta.png` をベースに全面描き直しを実施。通常時と完全に同一の画風・クオリティ・プロポーション（尖った三角立ち耳、シャープなV字胸毛、足元）を厳密に継承し、ゴンタのアイデンティティである「四角い緑フレームのメガネ＋完全透明レンズ＋澄んだ茶色い瞳」を100%維持しています。AI生成によるメガネの吊り上がりやサングラス化（黒塗りレンズ）、耳や体型の崩れを完全に排除しました。喜び（`joy`）のリアクションはクラッカーを胸元に構えて「パーン！」と鳴らし、カラフルな紙吹雪とテープが舞い散る専用アニメーションに描き直しています。また、`public/room-work-diary.png` の日記執筆姿についても同様にメガネの吊り上がり・サングラス化を修正し、クリアレンズの四角い眼鏡に変更しています。

### クラッカーの再生調整

`public/room-emotion-joy.png` の元の12コマ画像をそのまま使用しています。画像の描き直し・差し替えは行わず、`public/room.css` の再生順序と表示位置を修正しました。

- 体が二重になる2コマ目と、途中で最初の構えへ戻る4コマ目は再生から外します。
- 構える→向きを変える→発射→跳び上がる→着地→構えに戻る流れを4.2秒で再生します。発射は短く、前後の待ち時間を長めにしています。
- 5・7・8コマ目の下端に混ざっていた断片は、足元の外側だけをCSSで非表示にします。
- 9〜12コマ目の共通の位置ずれを26px相当補正し、ジャンプと着地の動きは残しながら床の高さを揃えます。
- 読み込み前／失敗時は共通の表情画像に戻り、「動きを減らす」設定では静止します。

素材を将来差し替える場合は、12列×1行の配置に加え、上記の補正値とコマ順も確認してください。
